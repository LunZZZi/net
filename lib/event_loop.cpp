...
int event_loop_handle_pending_add(struct event_loop *eventLoop, int fd, struct channel *channel) {
    net_msgx("add channel fd == %d, %s", fd, eventLoop->thread_name);
...
int event_loop_handle_pending_update(struct event_loop *eventLoop, int fd, struct channel *channel) {
    net_msgx("update channel fd == %d, %s", fd, eventLoop->thread_name);
    struct channel_map *map = eventLoop->channelMap;
...
int channel_event_activate(struct event_loop *eventLoop, int fd, int revents) {
    struct channel_map *map = eventLoop->channelMap;
    net_msgx("activate channel fd == %d, revents=%d, %s", fd, revents, eventLoop->thread_name);
...
int handleWakeup(void *data) {
    ...
    net_msgx("wakeup, %s", eventLoop->thread_name);
}
...
struct event_loop *event_loop_init_with_name(char *thread_name) {
    ...
    eventLoop->quit = 0;
    eventLoop->channelMap = (channel_map *)malloc(sizeof(struct channel_map));
    map_init(eventLoop->channelMap);

#ifdef EPOLL_ENABLE
    net_msgx("set epoll as dispatcher, %s", eventLoop->thread_name);
    eventLoop->eventDispatcher = &epoll_dispatcher;
#else
    net_msgx("set poll as dispatcher, %s", eventLoop->thread_name);
...
int event_loop_run(struct event_loop *eventLoop) {
    ...
    net_msgx("event loop run, %s", eventLoop->thread_name);
    ...
    net_msgx("event loop end, %s", eventLoop->thread_name);
    return 0;
}